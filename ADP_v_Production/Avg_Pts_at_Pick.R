...
#at the current pick x, we find percentage of points from picks x-5,x+5

buffer <- 3
max_adp <- 150
year_range <- 2019:2007
num_years <- length(year_range)
#contains num points found at pick i over all years
points_at_pick <- rep(0,max_adp)
smoothed_points_at_pick <- rep(0,max_adp)
...
for(i in year_range){
  ...
  for(j in 1:max_adp){
    ...
    if(!is.na(player_row[1,"Fan_Pts"])){
      #we found a starter at current adp in current year
      points_at_pick[j] <- points_at_pick[j]+player_row[1,"Fan_Pts"]
    }
  }
}
...
for(i in 1:buffer){
  smoothed_points_at_pick[i] <- sum(points_at_pick[1:(i+buffer)])
  num_picks_summed[i] <- length(1:(i+buffer))
}
for(i in (buffer+1):(max_adp-buffer)){
  smoothed_points_at_pick[i] <- sum(points_at_pick[(i-buffer):(i+buffer)])
  num_picks_summed[i] <- length((i-buffer):(i+buffer))
}
for(i in (max_adp-buffer+1):max_adp){
  smoothed_points_at_pick[i] <-sum(points_at_pick[(i-buffer):max_adp])
  num_picks_summed[i] <- length((i-buffer):max_adp)
}

smoothed_prob_of_starter <- smoothed_points_at_pick/(num_years*num_picks_summed)
